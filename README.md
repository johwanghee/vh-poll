...
| 구분 | 의미 |
| --- | --- |
| Persona 데이터에서 계산 | 득표율, 관점별 범위, 그룹별 개수와 변화량, 결측치와 관측 범위 |
| 시나리오 가정 | 어떤 속성이 어느 선택지를 지지한다고 본 방향과 효과 크기 |
| 강건성 판단 | 세 관점에서 승자가 유지되는지, 여러 관점에 반복된 신호가 있는지 |

속성과 선택지의 연결 방향은 데이터에서 학습한 상관관계나 인과효과가 아닙니다. LLM이 질문별로 만든 명시적인 시나리오 가정입니다. 반면 표시된 득표율과 범위는 그 규칙을 전체 Persona 데이터에 적용해 계산한 값입니다. 이 구분 덕분에 결과가 왜 달라졌는지 확인할 수 있지만, 현실의 사람이나 집단에 일반화할 근거가 되지는 않습니다.
